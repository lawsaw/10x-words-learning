...
export const categoriesQuerySchema = z.object({
  ...
  page: pageSchema,
  pageSize: z.coerce.number().int().min(1).max(50).default(10),
  orderBy: z.enum(['createdAt', 'name']).default('createdAt') as z.ZodType<CategoryOrderField>,
...
export const categoryWordsQuerySchema = z
  .object({
    view: z.enum(['table', 'slider']).default('table') as z.ZodType<WordViewMode>,
    orderBy: z
...
export const createWordCommandSchema = z.object({
  ...
  translation: z.string().min(1, 'Translation is required').max(500, 'Translation is too long'),
  transcription: z
    .string()
