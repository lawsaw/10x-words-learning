...
export class TestingResetService {
  ...
  static async resetDatabase(command: TestResetCommand): Promise<void> {
    ...
    try {
      ...
      await supabase.schema("app").from("words").delete().neq("id", "00000000-0000-0000-0000-000000000000")

      // Delete categories (depends on words via cascade)
      await supabase.schema("app").from("categories").delete().neq("id", "00000000-0000-0000-0000-000000000000")

      // Delete learning languages (depends on categories via cascade)
      await supabase.schema("app").from("user_learning_languages").delete().neq("id", "00000000-0000-0000-0000-000000000000")

      // Delete profiles (depends on learning languages via cascade)
      await supabase.schema("app").from("profiles").delete().neq("user_id", "00000000-0000-0000-0000-000000000000")
