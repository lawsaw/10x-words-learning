...
export class AddLearningLanguageDialog {
  ...
  constructor(page: Page) {
    ...
    this.dialog = page.getByTestId('add-learning-language-dialog').or(page.getByRole('dialog'))
  ...
  async selectLanguage(languageCode: string): Promise<void> {
    ...
    const languageOption = this.page
      .getByTestId(`language-option-${languageCode}`)
...
export class DeleteLearningLanguageDialog {
  ...
  constructor(page: Page) {
    ...
    // Dialog container with fallback to role-based selector
    this.dialog = page
      .getByTestId('delete-learning-language-dialog')
    ...
    this.confirmButton = page
      .getByTestId('dialog-confirm-delete')
  ...
  async deleteLanguage(waitForClose: boolean = true): Promise<void> {
    if (waitForClose) {
      await Promise.all([this.waitForClose(), this.clickConfirm()])
    } else {
