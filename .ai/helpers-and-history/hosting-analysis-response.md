...
- **Selection**:
  ...
  4. **Vercel** (The creator of Next.js, offering native, zero-config deployments)
  5. **Netlify** (A primary competitor with a specialized "Next.js Runtime" and strong commercial tiers)
  6. **AWS Amplify** (An enterprise-grade solution wrapping AWS services, offering managed Next.js hosting via its Gen 2 platform)
...
- **Selection**:
  ...
  3. **DigitalOcean App Platform** (A managed PaaS that supports Docker, offering predictable pricing and simple scaling)
  4. **Coolify** (An open-source, self-hosted PaaS installed on a VPS, offering a Vercel-like experience with no subscription fees beyond raw server costs)
...
- **Vercel**:
  ...
  - Cons: Pro plan ($20/user) applies for _any_ commercial usage. Limits on function execution time can be tight.
- **Netlify**:
  - Pros: Great DX.
