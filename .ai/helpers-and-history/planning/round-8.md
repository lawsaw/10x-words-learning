...
} 2) Not needed. 3) Not needed. 4) Do not display anything. If the JSON is invalid, simply remove the loader and allow the user to fill in the form manually. 5) Any operations. 6) Create a local or temporary testing database. 7) Not needed. 8) When clicking “Shuffle Cards,” the cards are shuffled and reset to the beginning. 9) No email confirmation is required. Password: minimum 6 characters. 10) Do nothing.
